            ...
            <div class="col-xl-12 col-lg-12 col-sm-12  layout-spacing">
                <div class="widget-content widget-content-area br-6">
                    ...
                    <div class="col 12 button-holder" style="display: flex">
                        ...
                        <a href="{{route('students.create')}}" class="create-button-btn btn btn-success mb-6 mr-4 btn-lg"> Create</a>
                        </div>
                    </div>
                    <table id="html5-extension" class="table table-hover non-hover" style="width:100%">
                        ...
                        @foreach($users as $user)
                            <tr>
                                ...
                                <td>{{$user->subscription('SERU')?->ends_at->format('d-M-Y')}}</td>
                                <td>{{$user->status}}</td>
                                <td>
                                    ...
                                    <a id="{{$user->student->id}}" class="btn btn-danger delete-car" data-value="{{$user->name}}" onClick="destroy_car(this.id)">
                                        <i class="far fa-trash-alt"></i>
                                    </a>
                                    @endif

                                        <a id="{{$user->id}}" class="btn btn-success delete-car" data-value="{{$user->name}}" onClick="add_days(this.id)">
...
@section('js-customization')
        ...
        // $('.delete-car').on('click', function () {
         function destroy_car(car_id) {
            ...
            swalWithBootstrapButtons({
                title: 'Are you sure you want to delete Student Named '+carName+'?',
    ...
    <script>
        ...
        function add_days(car_id) {
            ...
            swalWithBootstrapButtons({
                title: 'Are you sure you want to Add Days to Student '+carName+'?',
                text: "You won't be able to revert this!",
                type: 'warning',
                input: 'number',
                ...
                if (result.value) {
                    $.ajax({
                        url: '/admin/students/extend/'+car_id,
                        method: 'POST',
                        data:{"_token": "{{ csrf_token() }}", "add_days": result.value},
                        success: function(resp)
                        {
                            swalWithBootstrapButtons(
                                {
                                    title: 'Completed!',
                                    text: 'The date has been Extended.',
                                    type: 'success'
                                }
                            ).then(function (result){
                                location.reload();
